# Nity / Continuity — Hydraulic Constitution (Full, 24 Articles)
...
\[
stall(W)=\mathbb{1}\left[\mathrm{PSI}^{full}_r(W)>\theta^{full}_r\right] \lor \mathbb{1}\left[\mathrm{PSI}^{some}_r(W)>\theta^{some}_r\right]
\]
Sustained stall over discrete samples:
\[
stall\_sust=\mathbb{1}\left[\sum_{k=1}^{m}stall(t-k\Delta t)\ge m\cdot p\right]
\]
...
\[
stall(W)=\mathbb{1}[\mathrm{PSI}^{full}(W)>\theta^{full}] \lor \mathbb{1}[\mathrm{PSI}^{some}(W)>\theta^{some}]
\]
...
### Equations
\[
ready=\mathbb{1}[TTF_{node}>T_{min}] \land \mathbb{1}[stall\_sust=0] \land \mathbb{1}[FD^*<\mu] \land \mathbb{1}[CT^*<\nu]
\]
...
### Equations
\[
stale=\mathbb{1}[now-ts>TTL]
\]
\[
accept=\mathbb{1}[schema\_ver\in supported]\land \mathbb{1}[stale=0]
\]
...
Drop rule:
\[
drop=\mathbb{1}[depth\ge D_{max}] \lor \mathbb{1}[RSI>r_{max}]
\]
Wait time:
\[
...
### Equations
\[
allow=\mathbb{1}[policy(src,dst,svc)=allow]
\]
...
\[
score_{ttf}=\mathbb{1}[TTF_{system}<T_{safe}],\quad score_{psi}=stall\_sust,\quad score_{err}=\mathbb{1}[err\_rate>\eta]
\]
\[
